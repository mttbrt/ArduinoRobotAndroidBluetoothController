...
public class Sensors extends AppCompatActivity implements View.OnClickListener, SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor == forwardSensor && values[0] < event.values.length) {
            ...
            } else { // If played don't calibrate, just execute commands
        ...
        if (event.sensor == rightSensor && values[1] < event.values.length) {
            if(!isListeningSensors) { // If it's paused, record calibration
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_start_stop:
                if(isListeningSensors) { // Cannot record if it's calibrating
                    isRecording = false;
                    setRecordingIcon();
                }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_resetC:
                isListeningSensors = false; // Switch to calibration mode
                setPlayPauseIcon();
